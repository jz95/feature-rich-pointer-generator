...
class SummarizationModel(object):
    ...
    def _add_encoder(self, encoder_inputs, seq_len, mode='word'):
        ...
        """
        assert mode in ('word', 'pos', 'char')
        with tf.variable_scope('encoder-' + mode):
    ...
    def _reduce_states(self, fw_st_lst, bw_st_lst):
        ...
        Args:
          fw_st_lst: list of LSTMStateTuple with hidden_dim units.
          bw_st_lst: list of LSTMStateTuple with hidden_dim units.

        Returns:
          state: LSTMStateTuple with hidden_dim units.
        """
        hidden_dim = self._hps.hidden_dim
        assert len(fw_st_lst) == len(bw_st_lst)
        n = len(fw_st_lst)
        with tf.variable_scope('reduce_final_st'):
            ...
            tmp = []
            for i in range(n):
                tmp.append(fw_st_lst[i])
                tmp.append(bw_st_lst[i])

            old_c = tf.concat(axis=1, values=[tp.c for tp in tmp])
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('embedding'):
                ...
                if hps.how_to_use_pos != 'no':
                    embedding_pos = tf.get_variable('embedding_pos', [vsize, hps.pos_emb_dim], dtype=tf.float32, initializer=self.trunc_norm_init)
                    if hps.mode == "train":
            ...
            self._dec_in_state = self._reduce_states(fw_st_lst, bw_st_lst)
