...
class Example(object):
    ...
    def get_dec_inp_targ_seqs(self, sequence, max_len, start_id, stop_id):
        ...
        target = sequence[:]
        if len(inp) > max_len.value:  # truncate
            inp = inp[:max_len.value]
            target = target[:max_len.value]  # no end_token
        else:  # no truncation
...
class Batch(object):
    ...
    def init_encoder_seq(self, example_list, hps):
        ...
        self.enc_padding_mask = np.zeros(
            (hps.batch_size.value, max_enc_seq_len), dtype=np.float32)
...
class Batcher(object):
    ...
    def __init__(self, data_path, vocab, hps, single_pass):
        ...
        self._example_queue = Queue.Queue(
            self.BATCH_QUEUE_MAX * self._hps.batch_size.value)
